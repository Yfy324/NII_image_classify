from dataset_split import *
from img_fea_generator import *
from sift_bof import *

if __name__ == '__main__':
    original = '/home/yfy/Desktop/projects/ImageNet/dogs_vs_cats/'
    base = '/home/yfy/Desktop/projects/ImageNet/dogs_vs_cats/cats_and_dogs_small/'
    dir_creat(original, base)

    dic_path = '/home/yfy/Desktop/projects/ImageNet/dogs_vs_cats/cats_and_dogs_small/dic/'
    n_features = 64
    im_paths, im_classes = path_label(dic_path, 1)
    describe, dic, d_list = sift_descriptor_voc(im_paths, n_features)
    im_features, im_distribution = bof_features(im_paths, dic, d_list, n_features, 1)
    im_data = data_merge(im_classes, im_paths, im_features)
    joblib.dump((im_distribution, n_features, dic), "2c_bof_features.pkl", compress=3)

    train_path = '/home/yfy/Desktop/projects/ImageNet/dogs_vs_cats/cats_and_dogs_small/train/'
    im_distribution, n_features, dic = joblib.load('2c_bof_features.pkl')
    aug = 4
    aug_size = 1 + aug
    im_paths, im_classes = path_label(train_path, aug_size)
    d_list_tr = aug_generate_descriptors(im_paths, aug_size)
    for i in range(len(d_list_tr)):
        if d_list_tr[i] is None:
            d_list_tr[i] = d_list_tr[i-1]
            print("None appears", i)
    im_features = generate_features(im_paths, aug_size, d_list_tr, dic, n_features, im_distribution)
    data_train = data_merge(im_classes, im_paths, im_features)
    train1 = pd.DataFrame(data_train)
    train1.to_csv('4_train1.csv')

    val_path = '/home/yfy/Desktop/projects/ImageNet/dogs_vs_cats/cats_and_dogs_small/validation/'
    im_distribution, n_features, dic = joblib.load('2c_bof_features.pkl')
    b_size = 1
    im_paths_val, im_classes_val = path_label(val_path, b_size)
    d_list_val = generate_descriptors(im_paths_val)
    im_features_val = generate_features(im_paths_val, b_size, d_list_val, dic, n_features, im_distribution)
    data_val = data_merge(im_classes_val, im_paths_val, im_features_val)
    val1 = pd.DataFrame(data_val)
    val1.to_csv('1_val1.csv')
