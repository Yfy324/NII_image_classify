...
def img_descriptor(path, batch_size=4):
    image = read_img(path)
...
def sift_bof_features(dir_path, k, batch_size):
    ...
    for label_name in label_names:
        ...
        image_classes += [class_id] * len(class_path) * 4
        class_id += 1
    ...
    im_features = np.zeros((4*len(image_paths), k), "float32")
    for i in range(4*len(image_paths)):
        words, distance = vq(des_list[i], voc)
        for w in words:
            im_features[i][w] += 1   # 统计匹配某一特征的次数

    y = np.array(image_classes).reshape((4*len(image_paths), 1))
